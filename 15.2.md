...
В разделе `metadata` в качестве пользовательских данных используется файл [cloud-init.yaml](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/cloud-init.yaml) следующей конфигурации
```yml
#cloud-config
users:
  - name: leo
...
```
Разместить в стартовой веб-странице шаблонной ВМ ссылку на [картинку](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/image.jpg) из бакета.

Ссылка размещена в файле [cloud-init.yaml](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/cloud-init.yaml)
```tf
write_files:
  - path: /var/www/html/index.html
    content: |
...
Проверка состояния настроена в [main.tf](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/main.tf)
```tf
 health_check {
    interval = var.lamp_interval
    timeout  = var.lamp_timeout
...
Чтобы не возникло конфликтов, закомментируем раздел кода с балансировщиком, выполненном в рамках пункта 3 данной работы

![Alt_text](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/main/screenshots/cloud2.11.png)

Далее, чтобы избежать путаницы создадим `Application Load Balancer` в отдельном файле [load-balancer.tf](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/load-balancer.tf)
```tf
...
```
Тут частично использованы переменные для сетевого балансировщика, а частично хардкод (в данном случае он мне необходим, чтобы не было путпнницы в названиях и переменных).
...
![Alt_text](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/main/screenshots/cloud2.12.png)
![Alt_text](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/main/screenshots/cloud2.13.png)
...
Манифесты по результатам выполнения задания:

[main.tf](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/main.tf)

[load-balancer.tf](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/load-balancer.tf)

[variables.tf](https://github.com/LeonidKhoroshev/clopro-homeworks/blob/hw-15.2/variables.tf)
